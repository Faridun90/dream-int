    ...
    return (
      <div className="h-full flex gap-3 flex-col mx-auto items-center">
        <h2 className="text-3xl my-10">
          Admin page - Welcome back: <span className="font-semibold text-indigo-300">{session.data.user.username}</span>
        </h2>

        <form onSubmit={handleSubmit} className="flex flex-col gap-2  border-2 p-3 rounded-md">
          <input
            type="text"
            name="dream"
            value={inputValue}
          ...
          />
          <button type="submit" className="p-2 w-fit mx-auto hover:bg-indigo-400 border-2 border-indigo-400 rounded-md">Submit Dream</button>
        </form>
      </div>
    );
  }
